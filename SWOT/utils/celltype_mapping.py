...
def ct_mapping_non_zero_sum(T_mapping,
    ...
    """
    Mapping transport plan into cell-type proportions
    """
...
def ct_mapping(t_mapping,
    ...
    """
    Cell-type mapping for inferring cell-type composition of SWOT.
    """
    ...
    file_name = 'CT_mapping.csv'
    if save:
        files = os.listdir(file_path)
        if 'CTmapping' not in files:
            os.mkdir(os.path.join(file_path, 'CTmapping'))

        ct_mapping.to_csv(file_path + 'CTmapping/' + file_name, sep=',', index=True, header=True)
        print('The cell-type mapping results are saved in ' + file_path + 'CTmapping/' + file_name + '.')
...
def compu_cells_eachspot(number_method, st_exp, cells_eachspot=0, tech=None):
    """
    Estimation of cell numbers within each spot in cell mapping step of SWOT.
    """
...
def cell_mapping_xy(ct_mapping, st_xy, cellnum_spot):
    """
    Estimation of cell coordinates within each spot in cell mapping step of SWOT.
    :return:
            cs_xy: cell coordinates information with rows being cells and columns being
            ...
            cs_ctnum: the number of each cell type in each spot,
                      with rows being spots and columns being cell types.
    """
...
def cell_mapping_expression(t_mapping, cs_xy, cellnum_spot, sc_exp,
    ...
    """
    Obtain the cell expression profile， with rows being genes and columns being cells.
    """
    ...
    #cs_cells = pd.DataFrame(cs_cells)
    ...
    file_name = 'Cell_mapping_exp.csv'
    #file_name_cs_cells = 'cs_cells.csv'
    file_name_cs_xy = 'Cell_mapping_xy_new.csv'
    #file_name_cs_ctnum_new = 'cs_ctnum_new.csv'
    if save:
        files = os.listdir(file_path)
