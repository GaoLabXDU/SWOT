...
def init_distribution(cost12, method='minus'):
    """
    ...
    if method == 'minus_exp':
        ...
        sc_a = sc_a / np.sum(sc_a)
        st_b = st_b / np.sum(st_b)
    # Without any prior information, we set the probabilities to what we observe empirically:
    # uniform over all observed sample
...
def transport_plan(cost12,
                   file_path, save=True,
                   alpha=0.1, lamda=1.0, ent_reg=1.0,
                   spa_cost=None, cost1=None, cost2=None,
    ...
    # T_mapping = T_mapping.round(3)

    file_name = 'T_mapping.csv'
    if save:
