---
title: "Visualization for cell-type deconvolution and single-cell spatial maps of SWOT"
...
## Introduction
SWOT is a spatially weighted optimal transport method for cell-type deconvolution and further single-cell spatial maps reconstruction for spatial transcriptomics data.   
In this example, we will visualize the cell-type deconvolution results and the reconstructed single-cell spatial maps by SOWT in pancreatic ductal adenocarcinoma (PDAC) dataset. 
...
&emsp;&emsp;"Celltype_proportions.csv": cell-type proportions matrix for cell-type deconvolution;   
&emsp;&emsp;"Cell_maps_xy.csv": cell spatial coordinates matrix for reconstructing the single-cell spatial map;   
&emsp;&emsp;"Cell_maps_exp.csv": single-cell gene expression matrix for reconstructed single-cell spatial map.   


```{r}
file_path <- getwd()
data_path <- paste0(file_path, '/Data/PDAC/')
swot_path <- paste0(file_path, '/SWOT_files/')

ST_xy <- read.csv(paste0(data_path, 'st_xy.csv'), header = T, row.names = 1, check.names = F)

SWOT_CTprop <- read.csv(paste0(swot_path, '/CellMapping/Celltype_proportions.csv'), header = T, row.names = 1, check.names = F)
...
## Visualization of cell-type deconvolution results
The visualization of cell-type deconvolution results includes the depiction of the spatial probability distribution of each cell type and the spatial distribution of individual cell types.
...
## Visualization of single-cell spatial maps reconstruction results
For the reconstructed single-cell spatial map of PDAC dataset, we plotted the spatial distribution of cell types and the spatial gene expression patterns for individual cell types.
...
SC_exp <- read.csv(paste0(data_path, 'sc_exp.csv'), header = T, row.names = 1, check.names = F)
SC_meta <- read.csv(paste0(data_path, 'sc_meta.csv'), header = T, row.names = 1, check.names = F)
...
markers_top100 <- read.csv(paste0(data_path, 'markers_top100.csv'), header = T, row.names = 1, check.names = F) 

# Plot expression levels of top 100 cell-type-specific genes on the reconstructed single-cell spatial map of PDAC dataset.
